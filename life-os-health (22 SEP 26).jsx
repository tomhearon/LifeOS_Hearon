...
/* ── Theme system (from the Claude Design round, Sep 2026) ──
   Every color is a CSS custom property; a theme is a token set applied to :root.
   C maps the app's legacy names onto the tokens, so all existing styles re-theme instantly. */
const THEMES = {
  field: { label: "Field Ledger", tag: "DEFAULT", desc: "Paper base · console density · glow on live elements", sw: ["#93402A", "#E2725B", "#8B5E7B"], swBg: "#F8F4EB",
    t: { bg: "#F8F4EB", card: "#FFFDF7", card2: "#F1EADC", card3: "#E9DFCC", ink: "#211B14", sub: "#6E6254", mut: "#9A8570", line: "#E5DCCB", lineSoft: "#EDE5D4",
      acc: "#93402A", accInk: "#FFFFFF", accSoft: "rgba(147,64,42,0.10)", health: "#C1533B", money: "#B98A2F", admin: "#5E7B8B", learn: "#8B5E7B", learnBg: "rgba(139,94,123,0.12)",
      good: "#4E6B3E", goodBg: "rgba(110,139,94,0.14)", bad: "#C24A2E", pop: "#93402A", glow: "0 0 14px rgba(147,64,42,0.15)",
      fDisp: "'Newsreader', Georgia, serif", fBody: "system-ui, -apple-system, sans-serif" } },
  espresso: { label: "Espresso Command", tag: "", desc: "Warm dark · dense console · bright terracotta", sw: ["#FF7A55", "#FFB340", "#FF8FA8"], swBg: "#221912",
    t: { bg: "#1B1410", card: "#251C16", card2: "#2E231B", card3: "#382B20", ink: "#F4EADB", sub: "#B39C87", mut: "#7E6B5A", line: "#42332699", lineSoft: "#3A2D2266",
      acc: "#E8A64C", accInk: "#1B1410", accSoft: "rgba(232,166,76,0.13)", health: "#E2725B", money: "#FFB340", admin: "#7FB5A8", learn: "#D98594", learnBg: "rgba(217,133,148,0.13)",
      good: "#9DBE8D", goodBg: "rgba(157,190,141,0.13)", bad: "#D06248", pop: "#FFB454", glow: "0 0 16px rgba(255,122,85,0.18)",
      fDisp: "'Space Grotesk', system-ui, sans-serif", fBody: "Inter, system-ui, -apple-system, sans-serif" } },
  ledger: { label: "The Ledger", tag: "", desc: "Warm paper · serif numerals · broadsheet calm", sw: ["#93402A", "#B98A2F", "#6E8B5E"], swBg: "#F8F4EB",
    t: { bg: "#F8F4EB", card: "#F8F4EB", card2: "#F1EADC", card3: "#E9DFCC", ink: "#211B14", sub: "#6E6254", mut: "#9A8570", line: "#E5DCCB", lineSoft: "#EDE5D4",
      acc: "#93402A", accInk: "#FFFFFF", accSoft: "rgba(147,64,42,0.10)", health: "#93402A", money: "#B98A2F", admin: "#5E7B8B", learn: "#8B5E7B", learnBg: "rgba(139,94,123,0.12)",
      good: "#4E6B3E", goodBg: "rgba(110,139,94,0.14)", bad: "#C24A2E", pop: "#B98A2F", glow: "none",
      fDisp: "'Newsreader', Georgia, serif", fBody: "system-ui, -apple-system, sans-serif" } },
  midnight: { label: "Midnight Signal", tag: "", desc: "Green-black · multi-neon glow · glassy cards", sw: ["#3DF0A4", "#4DD8FF", "#C77DFF"], swBg: "#081009",
    t: { bg: "#081009", card: "#0F1D15", card2: "#142A1E", card3: "#1A3526", ink: "#E9F5EC", sub: "#8FB39D", mut: "#567A64", line: "#1F3B2C", lineSoft: "#16301F",
      acc: "#3DF0A4", accInk: "#081009", accSoft: "rgba(61,240,164,0.10)", health: "#3DF0A4", money: "#4DD8FF", admin: "#FFC24B", learn: "#C77DFF", learnBg: "rgba(199,125,255,0.12)",
      good: "#3DF0A4", goodBg: "rgba(61,240,164,0.10)", bad: "#F07B5B", pop: "#E4FF5F", glow: "0 0 18px rgba(61,240,164,0.22)",
      fDisp: "'Sora', system-ui, sans-serif", fBody: "'Sora', system-ui, sans-serif" } },
  atlas: { label: "Obsidian Atlas", tag: "GRAPHICS", desc: "Deep slate · cyan glow · glass cards, photo-ready", sw: ["#3DD6F5", "#54E3A2", "#B08CFF"], swBg: "#0B0E13",
    t: { bg: "#0B0E13", card: "#131820", card2: "#1A2029", card3: "#212936", ink: "#EAF0F6", sub: "#7C8A99", mut: "#586674", line: "#232B36", lineSoft: "#1B222C",
      acc: "#3DD6F5", accInk: "#0B0E13", accSoft: "rgba(61,214,245,0.10)", health: "#54E3A2", money: "#3DD6F5", admin: "#F5D93D", learn: "#B08CFF", learnBg: "rgba(176,140,255,0.12)",
      good: "#54E3A2", goodBg: "rgba(84,227,162,0.12)", bad: "#F07B5B", pop: "#54E3A2", glow: "0 0 18px rgba(61,214,245,0.22)",
      fDisp: "'Sora', system-ui, sans-serif", fBody: "'Sora', system-ui, sans-serif" } },
};
const applyTheme = (id) => {
  const t = (THEMES[id] || THEMES.espresso).t;
  const r = document.documentElement;
  Object.entries(t).forEach(([k, v]) => r.style.setProperty("--" + k, v));
};
const C = {
  ground: "var(--bg)", panel: "var(--card)", panel2: "var(--card2)", panel3: "var(--card3)",
  rule: "var(--line)", ruleSoft: "var(--lineSoft)",
  ink: "var(--ink)", dim: "var(--sub)", dimmer: "var(--mut)",
  terra: "var(--health)", terraSoft: "var(--accSoft)",
  camel: "var(--acc)", camelSoft: "var(--accSoft)",
  rose: "var(--learn)", roseSoft: "var(--learnBg)",
  sage: "var(--good)", sageSoft: "var(--goodBg)",
  teal: "var(--admin)",
  neon: "var(--pop)",
  behind: "var(--bad)",
  cream: "var(--ink)",
...
const SANS = "var(--fBody)";
const DISP = "var(--fDisp)";
...
const SESSIONS_2 = {
  lowerA: { name: "Lower A — Quad emphasis · Block 2", color: "terra", ex: [
  ...
  upperA: { name: "Upper A — Push emphasis · Block 2", color: "camel", ex: [
  ...
  lowerB: { name: "Lower B — Hinge emphasis · Block 2", color: "rose", ex: [
  ...
  upperB: { name: "Upper B — Pull emphasis · Block 2", color: "sage", ex: [
...
/* ── Substitution system ──
   SUB_LIB: exercises that exist only as substitutes. SUB_BANK: per-exercise curated options
   (keys may point into SUB_LIB or into the session sets — e.g. Phase C cable work). */
const SUB_LIB = {
  machlat: { n: "Machine lateral raise", eq: "Machine", inc: 1, m: ["shoulders"] },
  platelat: { n: "Plate lateral raise", eq: "Other", inc: 1, m: ["shoulders"] },
  pecdeck: { n: "Pec deck", eq: "Machine", inc: 2.5, m: ["chest"] },
  cableoh1: { n: "One-arm cable overhead extension", eq: "Cable", inc: 1, m: ["triceps"] },
  ropepush: { n: "Rope pushdown", eq: "Cable", inc: 1, m: ["triceps"] },
  machpress: { n: "Machine chest press", eq: "Machine", inc: 2.5, m: ["chest", "triceps"] },
  smithbench: { n: "Smith machine bench press", eq: "Machine", inc: 2.5, m: ["chest", "triceps"] },
  bbbench: { n: "Barbell bench press", eq: "Barbell", inc: 2.5, m: ["chest", "triceps"] },
  machincl: { n: "Incline machine press", eq: "Machine", inc: 2.5, m: ["chest", "shoulders"] },
  machohp: { n: "Machine shoulder press", eq: "Machine", inc: 2.5, m: ["shoulders"] },
  bbohp: { n: "Barbell overhead press", eq: "Barbell", inc: 2.5, m: ["shoulders"] },
  tbarrow: { n: "T-bar row", eq: "Machine", inc: 2.5, m: ["upperback", "lats"] },
  bbrow: { n: "Barbell row", eq: "Barbell", inc: 2.5, m: ["lats", "upperback"] },
  dbpull: { n: "DB pullover", eq: "DB", inc: 2.5, m: ["lats", "chest"] },
  assistpull: { n: "Assisted pull-up machine", eq: "Machine", inc: 2.5, m: ["lats"] },
  machrear: { n: "Reverse pec deck", eq: "Machine", inc: 1, m: ["shoulders", "upperback"] },
  gobsq: { n: "Goblet squat", eq: "DB", inc: 2.5, m: ["quads", "glutes"] },
  smithsq: { n: "Smith machine squat", eq: "Machine", inc: 2.5, m: ["quads", "glutes"] },
  legext: { n: "Leg extension", eq: "Machine", inc: 2.5, m: ["quads"] },
  dbdl: { n: "Heavy DB deadlift", eq: "DB", inc: 2.5, m: ["hamstrings", "glutes"] },
  rackpull: { n: "Rack pull", eq: "Barbell", inc: 5, m: ["hamstrings", "glutes", "upperback"] },
  trapbar: { n: "Trap-bar deadlift", eq: "Barbell", inc: 5, m: ["hamstrings", "glutes", "quads"] },
  pullthru: { n: "Cable pull-through", eq: "Cable", inc: 2.5, m: ["glutes", "hamstrings"] },
  cablekick: { n: "Cable glute kickback", eq: "Cable", inc: 1, m: ["glutes"] },
  slidecurl: { n: "Sliding leg curl", eq: "Other", inc: 0, m: ["hamstrings"] },
  legpresscalf: { n: "Leg-press calf raise", eq: "Machine", inc: 5, m: ["calves"] },
  cablecurl: { n: "Cable curl", eq: "Cable", inc: 1, m: ["biceps"] },
  inclcurl: { n: "Incline DB curl", eq: "DB", inc: 1, m: ["biceps"] },
  cablehammer: { n: "Rope hammer curl", eq: "Cable", inc: 1, m: ["biceps"] },
  declinesit: { n: "Decline sit-up", eq: "Other", inc: 0, m: ["core"] },
  dbshrug: { n: "DB shrug", eq: "DB", inc: 2.5, m: ["upperback", "grip"] },
  cgbench: { n: "Close-grip bench press", eq: "Barbell", inc: 2.5, m: ["triceps", "chest"] },
};
const SUB_BANK = {
  backsq: ["hacksq", "smithsq", "gobsq", "legpress"], bss: ["stepup2", "fwdlunge", "smithsq"],
  legpress: ["hacksq", "legext", "gobsq"], rdl: ["sldl", "pullthru", "gluteham"],
  calf: ["seatedcalf", "legpresscalf"], kneeraise: ["cablecrunch", "declinesit"],
  bench: ["bbbench", "machpress", "smithbench"], ohp: ["machohp", "bbohp", "landmine"],
  csrow: ["sealrow", "cablerow", "tbarrow", "bbrow"], fly: ["cablefly", "pecdeck"],
  latraise: ["cablelat", "machlat", "platelat"], triext: ["cableoh1", "ropepush", "skull"],
  trapdl: ["trapbar", "rackpull", "dbdl"], revlunge: ["walklunge", "fwdlunge", "stepup2"],
  hipthrust: ["gluteham", "cablekick", "pullthru"], legcurl: ["nordic", "slidecurl", "gluteham"],
  stepup: ["stepup2", "legext"], pulldown: ["pullup", "assistpull", "dbpull"],
  incline: ["machincl", "cablefly", "landmine"], onearm: ["cablerow", "sealrow", "tbarrow"],
  arnold: ["machohp", "landmine", "bbohp"], reardelt: ["facepull", "machrear"],
  hammer: ["cablehammer", "ezcurl", "inclcurl"],
  walklunge: ["revlunge", "fwdlunge"], hacksq: ["gobsq", "smithsq", "legpress"],
  sldl: ["rdl", "pullthru"], seatedcalf: ["calf", "legpresscalf"], cablecrunch: ["kneeraise", "declinesit"],
  landmine: ["machohp", "ohp"], sealrow: ["cablerow", "tbarrow", "csrow"], cablefly: ["fly", "pecdeck"],
  cablelat: ["latraise", "machlat"], skull: ["cableoh1", "ropepush", "triext"],
  fwdlunge: ["revlunge", "walklunge"], gluteham: ["hipthrust", "pullthru"], nordic: ["legcurl", "slidecurl"],
  stepup2: ["stepup", "bss"], pullup: ["pulldown", "assistpull"], cablerow: ["sealrow", "tbarrow", "csrow"],
  facepull: ["machrear", "reardelt"], ezcurl: ["cablecurl", "hammer", "inclcurl"],
};
/* Master lookup: every exercise def by key (sessions first, then sub-only lib) */
const EX_LIB = {};
[SESSIONS_1, SESSIONS_2].forEach((set) => Object.values(set).forEach((s) => s.ex.forEach((e) => { EX_LIB[e.k] = e; })));
Object.keys(SUB_LIB).forEach((k) => { if (!EX_LIB[k]) EX_LIB[k] = { k, ...SUB_LIB[k] }; EX_INC[k] = EX_INC[k] ?? SUB_LIB[k].inc; });

/* ── Block rotation: exercises change every ~3 weeks (plateau + boredom insurance).
   B1 wk1–3 upper/lower · B2 wk4–6 upper/lower variants (incl. RESET) · then the split itself
   changes: Push / Pull / Legs / Shoulders+Arms for B3–B5, schemes shifting per block.
   Slot keys stay lowerA/upperA/lowerB/upperB so the scheduler never changes;
   anchor lifts (bench, trapdl=deadlift, backsq) keep their keys through every block. */
const X = (k, sets, reps, extra = {}) => ({ ...(EX_LIB[k] || { k, n: k }), k, sets, reps, ...extra });
const PPL_A = {
  lowerA: { name: "Push — Block 3", color: "terra", ex: [
    X("bench", 4, "8–10"), X("machincl", 3, "10–12"), X("cablefly", 3, "12–15"),
    X("pecdeck", 3, "12–15"), X("skull", 3, "10–12"), X("ropepush", 3, "12–15") ]},
  upperA: { name: "Pull — Block 3", color: "camel", ex: [
    X("trapdl", 4, "6–8"), X("pullup", 4, "6–10"), X("cablerow", 3, "10–12"),
    X("facepull", 3, "15"), X("ezcurl", 3, "10–12"), X("suitcase", 3, "40 m /side", { unit: "m", inc: 0 }) ]},
  lowerB: { name: "Legs — Block 3", color: "rose", ex: [
    X("backsq", 4, "8–10"), X("walklunge", 3, "10 /leg"), X("sldl", 3, "10–12"),
    X("legcurl", 3, "12–15"), X("seatedcalf", 4, "15–20"), X("cablecrunch", 3, "12–15") ]},
  upperB: { name: "Shoulders & Arms — Block 3", color: "sage", ex: [
    X("arnold", 4, "8–10"), X("cablelat", 3, "12–15"), X("machrear", 3, "15"),
    X("inclcurl", 3, "10–12"), X("cableoh1", 3, "10–12"), X("farmer", 3, "40 m", { unit: "m", inc: 0 }) ]},
};
const PPL_B = {
  lowerA: { name: "Push — Block 4 · heavier", color: "terra", ex: [
    X("bench", 4, "6–8"), X("incline", 3, "8–10"), X("machpress", 3, "10–12"),
    X("cgbench", 3, "6–8"), X("pecdeck", 3, "12–15") ]},
  upperA: { name: "Pull — Block 4 · heavier", color: "camel", ex: [
    X("trapdl", 4, "5–6"), X("bbrow", 3, "6–8"), X("pulldown", 3, "8–10"),
    X("dbshrug", 3, "10–12"), X("hammer", 3, "8–10"), X("suitcase", 3, "40 m /side", { unit: "m", inc: 0 }) ]},
  lowerB: { name: "Legs — Block 4 · heavier", color: "rose", ex: [
    X("backsq", 4, "6–8"), X("hacksq", 3, "8–10"), X("rdl", 3, "8–10"),
    X("nordic", 3, "6–8"), X("calf", 4, "12–15"), X("kneeraise", 3, "12–15") ]},
  upperB: { name: "Shoulders & Arms — Block 4 · heavier", color: "sage", ex: [
    X("ohp", 4, "6–8"), X("latraise", 3, "12–15"), X("facepull", 3, "15"),
    X("cablecurl", 3, "10–12"), X("skull", 3, "8–10"), X("farmer", 3, "40 m", { unit: "m", inc: 0 }) ]},
};
const PPL_C = {
  lowerA: { name: "Push — Block 5 · finisher", color: "terra", ex: [
    X("bench", 4, "10–12"), X("cablefly", 3, "12–15"), X("machincl", 3, "12–15"),
    X("landmine", 3, "10–12"), X("cableoh1", 3, "12–15") ]},
  upperA: { name: "Pull — Block 5 · finisher", color: "camel", ex: [
    X("trapdl", 3, "8–10"), X("sealrow", 3, "10–12"), X("pulldown", 3, "12–15"),
    X("dbpull", 3, "12–15"), X("cablehammer", 3, "12–15"), X("suitcase", 3, "40 m /side", { unit: "m", inc: 0 }) ]},
  lowerB: { name: "Legs — Block 5 · finisher", color: "rose", ex: [
    X("backsq", 3, "10–12"), X("gobsq", 3, "12–15"), X("gluteham", 3, "12–15"),
    X("legcurl", 3, "15"), X("seatedcalf", 4, "20"), X("cablecrunch", 3, "15") ]},
  upperB: { name: "Shoulders & Arms — Block 5 · finisher", color: "sage", ex: [
    X("machohp", 3, "10–12"), X("cablelat", 3, "15"), X("reardelt", 3, "15"),
    X("ezcurl", 3, "12–15"), X("ropepush", 3, "12–15"), X("farmer", 3, "40 m", { unit: "m", inc: 0 }) ]},
};
const sessionsFor = (date) =>
  date >= "2026-11-30" ? PPL_C : date >= "2026-11-09" ? PPL_B :
  date >= "2026-10-19" ? PPL_A : date >= "2026-09-28" ? SESSIONS_2 : SESSIONS_1;
...
const SCHED_1 = {
  ...
  2: { mob: "quick", key: "upperA", cardio: "Zone 2 · easy 6–8 km run or 40 min bike", kcal: 2450, carbs: 260 },
...
const EVENTS = {
  ...
  "2026-09-28": "Block 2 begins — same upper/lower split, rotated accessories. New movements carry fresh prompts; the big three keep their history.",
  "2026-10-19": "Block 3: the split itself changes — Push / Pull / Legs / Shoulders+Arms, 8–12 rep hypertrophy schemes. Deadlift moves to Tuesday (Pull).",
  "2026-11-09": "Block 4 — heavier: 5–8 rep schemes on the anchors; barbell rows and close-grip bench arrive. Phoenix falls in this block; travel kit covers it.",
  "2026-11-30": "Block 5 — the finisher: higher-rep, joint-friendly work through party season, arriving at the 20th strong, not fried.",
  ...
  return (
    <div style={{ display: "flex", alignItems: "baseline", gap: 5 }}>
      <input type="number" inputMode="decimal" step="any"
        value={editing ? txt : (value ?? "")} placeholder={placeholder}
        onChange={(e) => {
          const raw = e.target.value; setTxt(raw);
          onChange(raw === "" ? null : Number(raw));
        }}
        onFocus={(e) => { setTxt(value ?? ""); setEditing(true); e.target.style.borderColor = C.camel; }}
        onBlur={(e) => { setEditing(false); e.target.style.borderColor = "transparent"; }}
...
export default function LifeOS() {
  ...
  const themeId = data?.theme || (data?.autoDark === false ? "field"
    : (new Date().getHours() >= 19 || new Date().getHours() < 7 ? "midnight" : "field"));
  useEffect(() => { applyTheme(data?.theme
    ? data.theme
    : (data?.autoDark === false ? "field" : (new Date().getHours() >= 19 || new Date().getHours() < 7 ? "midnight" : "field")));
  }, [data?.theme, data?.autoDark]);
    ...
    <div style={{ background: `radial-gradient(1200px 500px at 50% -10%, #2A1E15 0%, ${C.ground} 60%)`,
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "22px 16px 0" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
          <div>
            <div style={{ fontFamily: DISP, fontSize: 24, fontWeight: 600, letterSpacing: "-0.01em" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <button onClick={() => setShowSettings(true)} title="Themes — the five presets from the Design round, plus auto dark mode"
              style={{ background: C.panel2, border: "none", borderRadius: 10, color: C.dim,
                fontFamily: MONO, fontSize: 11, padding: "8px 12px", cursor: "pointer" }}>◐</button>
        ...
        <div style={{ display: "flex", marginTop: 20, gap: 6, overflowX: "auto", paddingBottom: 2 }}>
          {[["home", "Home"], ["today", "Today"], ["train", "Train"], ["lifts", "Lifts"], ["fuel", "Fuel"], ["trend", "Trend"], ["de", "Deutsch"], ["body", "Body"], ["tests", "Tests"], ["mind", "Mind"]].map(([k, l]) => (
        ...
        {/* ══════ HOME ══════ */}
        {tab === "home" && (() => {
          const e = data.days[today] || {};
          const tSched = cycleFor(today) ? cycleFor(today).sched[parse(today).getDay()] : null;
          const sName = tSched?.key ? sessionsFor(today)[tSched.key].name : null;
          const st = data.deCards || {};
          const deDue = Math.min(DE_CARDS.filter((c) => st[c.id] && st[c.id].due <= today).length
            + DE_CARDS.filter((c) => !st[c.id]).length, 10);
          const daysLeft = dayDiff(today, "2026-12-20");
          const lifted = !!(data.lifts[today] && Object.keys(data.lifts[today]).length);
          const orders = [
            { l: "Morning weigh-in", done: e.weight != null, go: "today", c: C.terra },
            { l: "Creatine + IM8", done: !!(e.creatine && e.im8), go: "today", c: C.terra },
            sName ? { l: sName.split(" — ")[0] + " · lift", done: lifted, go: "train", c: C.terra } : null,
            tSched?.cardio ? { l: tSched.cardio, done: !!(e.cardio || e.cardioMin), go: "today", c: C.teal } : null,
            tSched?.hiit && (cWeek == null || cWeek >= 3) ? { l: "Hill sprints", done: !!e.hiit, go: "train", c: C.behind } : null,
            { l: "Mobility · " + (MOBILITY[tSched?.mob]?.name.split(" ·")[0] || "flow"), done: !!e.mob, go: "train", c: C.teal },
            { l: "Deutsch · Lektion + Quiz", done: !!e.quizDe, go: "de", c: C.rose },
            { l: `Deutsch · Vokabeln (${deDue} due)`, done: deDue === 0, go: "de", c: C.rose },
            { l: "Latina · daily quiz", done: !!e.quizLa, go: "mind", c: C.camel },
            { l: "Affirmation", done: !!(data.affirm && data.affirm[today]), go: "mind", c: C.rose },
          ].filter(Boolean);
          const doneN = orders.filter((o) => o.done).length;
          const applets = [
            { t: "TRAIN", c: C.terra, big: sName ? sName.split(" — ")[0] : "Rest / cardio", sub: tSched?.cardio || (sName ? "Gym day" : "Recovery"), go: "train" },
            { t: "FUEL", c: C.camel, big: `${e.kcal ?? "—"} / ${tSched?.kcal ?? "—"}`, sub: `Protein ${e.pro ?? "—"} / ${cyc.protein} g`, go: "fuel" },
            { t: "TREND", c: C.sage, big: latest?.avg ? `${latest.avg.toFixed(1)} kg` : (e.weight ? `${e.weight} kg` : "—"), sub: verdict ? `${verdict.rate.toFixed(2)} kg/wk` : "7-day average", go: "trend" },
            { t: "DEUTSCH", c: C.rose, big: `${daysLeft} Tage`, sub: `${deDue} Vokabeln due · ${DE_PHASES[Math.max(0, Math.min(4, parse(today).getMonth() - 7))][1]}`, go: "de" },
            { t: "MIND", c: C.teal, big: ROME[doy % ROME.length].la.length > 26 ? ROME[doy % ROME.length].la.slice(0, 26) + "…" : ROME[doy % ROME.length].la, sub: "Today's quotations · journal", go: "mind" },
            { t: "LIFTS", c: C.dim, big: "Records", sub: "PRs · body map", go: "lifts" },
          ];
          return (
            <div style={{ marginTop: 22, display: "grid", gap: 14 }}>
              <Panel glow style={{ borderLeft: "3px solid var(--acc)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                  <Label tip="Everything the day asks of you, drawn live from the plan — ✓ fills in as you log. Tap any line to jump to where it happens." style={{ marginBottom: 0 }}>
                    Today's orders · {doneN}/{orders.length}</Label>
                  <span style={{ fontFamily: MONO, fontSize: 10, color: doneN === orders.length ? C.sage : C.dimmer }}>
                    {doneN === orders.length ? "ALL CLEAR ✓" : pretty(today).toUpperCase()}</span>
                </div>
                <div style={{ marginTop: 8 }}>
                  {orders.map((o, i) => (
                    <div key={i} onClick={() => setTab(o.go)} style={{ display: "flex", gap: 10, alignItems: "center",
                      padding: "9px 0", borderBottom: i < orders.length - 1 ? `1px dotted ${C.ruleSoft}` : "none", cursor: "pointer" }}>
                      <span style={{ fontFamily: MONO, fontSize: 12, color: o.done ? C.sage : o.c, width: 15, flexShrink: 0 }}>
                        {o.done ? "✓" : "○"}</span>
                      <span style={{ fontSize: 13.5, color: o.done ? C.dimmer : C.ink,
                        textDecoration: o.done ? "line-through" : "none", flex: 1 }}>{o.l}</span>
                      <span style={{ fontFamily: MONO, fontSize: 10, color: C.dimmer }}>→</span>
                    </div>))}
                </div>
              </Panel>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 12 }}>
                {applets.map((a) => (
                  <div key={a.t} onClick={() => setTab(a.go)} style={{ background: C.panel, borderRadius: 14,
                    border: `1px solid ${C.rule}`, borderLeft: `3px solid ${a.c}`, padding: "13px 15px", cursor: "pointer" }}>
                    <div style={{ fontFamily: MONO, fontSize: 9, letterSpacing: "0.14em", color: C.dimmer }}>{a.t}</div>
                    <div style={{ fontFamily: DISP, fontSize: 17, fontWeight: 600, marginTop: 5, lineHeight: 1.25 }}>{a.big}</div>
                    <div style={{ fontSize: 11, color: C.dim, marginTop: 3 }}>{a.sub}</div>
                    <div style={{ fontFamily: MONO, fontSize: 10, color: a.c, marginTop: 8 }}>OPEN →</div>
                  </div>))}
              </div>
              {EVENTS[today] && (
                <Panel style={{ borderLeft: "3px solid var(--acc)" }}>
                  <div style={{ fontSize: 13, lineHeight: 1.6 }}>📌 {EVENTS[today]}</div>
                </Panel>)}
            </div>);
        })()}
              ...
              <Panel>
                ...
                {sched.cardio && (
                  <div style={{ marginTop: 14, padding: "11px 13px", borderRadius: 12, background: C.panel2 }}>
                    <Label tip="Detail for the day's cardio session — type, duration, distance, average HR. Pace computes automatically; the pace-at-fixed-HR trend is your clearest fitness signal for the week-5 and week-15 tests." style={{ marginBottom: 8 }}>
                      Cardio log · {sched.cardio}</Label>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
                      {["Intervals", "Steady", "Tempo", "Hills", "Ruck"].map((t) => (
                        <button key={t} onClick={() => setDay(cursor, "cardioT", entry.cardioT === t ? null : t)}
                          style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.06em", padding: "5px 11px",
                            borderRadius: 12, border: "none", cursor: "pointer",
                            background: entry.cardioT === t ? "var(--acc)" : C.panel3,
                            color: entry.cardioT === t ? "var(--accInk)" : C.dim }}>{t}</button>))}
                    </div>
                    ...
                    {entry.cardioMin > 0 && entry.cardioKm > 0 && (
                      <div style={{ marginTop: 8, fontFamily: MONO, fontSize: 11, color: "var(--acc)" }}>
                        {(entry.cardioMin / entry.cardioKm).toFixed(2)} min/km
                        {entry.cardioHr ? ` · ${entry.cardioHr} avg bpm` : ""}</div>)}
                    {(() => {
                      const recent = [];
                      for (let i = 1; i <= 21 && recent.length < 3; i++) {
                        const d = addDays(cursor, -i); const e = data.days[d];
                        if (e?.cardioMin && e?.cardioKm) recent.push({ d, e });
                      }
                      return recent.length ? (
                        <div style={{ marginTop: 10, borderTop: `1px solid ${C.ruleSoft}`, paddingTop: 4 }}>
                          {recent.map(({ d, e }) => (
                            <div key={d} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0",
                              borderBottom: `1px dotted ${C.ruleSoft}`, alignItems: "baseline" }}>
                              <span style={{ fontSize: 12 }}>{e.cardioT || "Run"}
                                <span style={{ fontFamily: MONO, fontSize: 9, color: C.dimmer, marginLeft: 6 }}>{pretty(d)}</span></span>
                              <span style={{ fontFamily: MONO, fontSize: 11, color: C.dim }}>
                                {e.cardioMin} min · {e.cardioKm} km · {(e.cardioMin / e.cardioKm).toFixed(1)} min/km{e.cardioHr ? ` · ${e.cardioHr}` : ""}</span>
                            </div>))}
                        </div>) : null;
                    })()}
                  </div>
            ...
            {!session ? (
              ...
              <Panel style={{ borderLeft: `3px solid ${colorOf(session.color)}` }}>
                ...
                <div style={{ display: "grid", gap: 10 }}>
                  {session.ex.map((ex) => {
                    const subKey = !entry.kit ? entry.subs?.[ex.k] : null;
                    const subDef = subKey ? (EX_LIB[subKey] || data.customEx?.[subKey]) : null;
                    const eff = subDef
                      ? { ...ex, k: subKey, n: subDef.n, inc: subDef.inc ?? 2.5, m: subDef.m || ex.m,
                          tip: subDef.tip || `Substitute for ${ex.n} — same sets and reps, its own record.` }
                      : ex;
                    const exKey = entry.kit ? ex.k + "~t" : eff.k;
                    ...
                    const chooseSub = (k) => {
                      ...
                      persist({ ...data,
                        days: { ...data.days, [cursor]: { ...day, subs: { ...(day.subs || {}), [ex.k]: k } } },
                        subMemory: { ...(data.subMemory || {}), [ex.k]: k } });
                      setSwapFor(null); setEqSel(null); setCustomTxt("");
                    };
                    const clearSub = () => {
                      const day = data.days[cursor] || {};
                      const subs = { ...(day.subs || {}) }; delete subs[ex.k];
                      persist({ ...data, days: { ...data.days, [cursor]: { ...day, subs } } });
                    };
                    const bankKeys = SUB_BANK[ex.k] || [];
                    const memKey = data.subMemory?.[ex.k];
                    const options = [...new Set([...(memKey && !bankKeys.includes(memKey) ? [memKey] : []), ...bankKeys])]
                      .map((k) => ({ k, ...(EX_LIB[k] || data.customEx?.[k] || { n: k, eq: "Custom" }) }));
                    const eqs = [...new Set(options.map((o) => o.eq || "Other"))];
                      ...
                      <div key={ex.k} style={{ borderRadius: 12, background: C.panel2, overflow: "hidden",
                        ...
                        <div onClick={() => setOpenEx(isOpen ? null : ex.k)} style={{
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 14, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
                              {entry.kit ? (ex.a || (eff.n + " — band/BW")) : eff.n}<Info tip={entry.kit ? (ex.at || "No authored travel variant — mirror the movement pattern with a band or bodyweight, matched by feel.") : eff.tip} /><DemoLink q={entry.kit ? (ex.a || eff.n + " band") : eff.n} />
                              {subKey && <span style={{ fontFamily: MONO, fontSize: 9, color: C.rose, letterSpacing: "0.06em" }}>SUB FOR {ex.n.toUpperCase()}</span>}</div>
                        ...
                        {isOpen && (
                          <div style={{ padding: "0 14px 14px", display: "grid", gap: 8 }}>
                            ...
                            {!entry.kit && !isDeload && prog.last && (
                              <button onClick={() => {
                                  const top = Math.max(...prog.last.rec.sets.filter((s) => s.load != null).map((s) => s.load));
                                  const load = prog.label.startsWith("+") ? top + eff.inc : (prog.label === "RE-BASE" ? Math.round(top * 0.9 * 4) / 4 : top);
                                  const reps = parseInt(String(eff.reps)) || 8;
                                  const day = { ...(data.lifts[cursor] || {}) };
                                  day[exKey] = { sets: Array.from({ length: nSets }, () => ({ load, reps })) };
                                  persist({ ...data, lifts: { ...data.lifts, [cursor]: day } });
                                }} style={{ width: "100%", padding: "9px 0", borderRadius: 10, border: `1px dashed ${C.rule}`,
                                  background: "transparent", color: C.dim, fontFamily: MONO, fontSize: 10,
                                  letterSpacing: "0.1em", cursor: "pointer" }}>
                                ✓ AS PLANNED — FILL {nSets} SETS ({prog.label.startsWith("+") ? "last top + " + eff.inc : prog.label === "RE-BASE" ? "−10% of last top" : "match last top"}, edit after)</button>)}
                            {!entry.kit && (
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                                {subKey
                                  ? <button onClick={clearSub} style={{ background: "transparent", border: `1px solid ${C.rule}`,
                                      borderRadius: 10, color: C.dim, fontFamily: MONO, fontSize: 10, padding: "7px 11px",
                                      cursor: "pointer" }}>↩ BACK TO {ex.n.toUpperCase()}</button>
                                  : <button onClick={() => { setSwapFor(swapFor === ex.k ? null : ex.k); setEqSel(null); setCustomTxt(""); }}
                                      style={{ background: C.panel3, border: "none", borderRadius: 10, color: C.camel,
                                        fontFamily: MONO, fontSize: 10, padding: "7px 11px", cursor: "pointer",
                                        letterSpacing: "0.06em" }}>⇄ NOT AVAILABLE?</button>}
                                <RestTimer />
                              </div>)}
                            {entry.kit && <div style={{ display: "flex", justifyContent: "flex-end" }}><RestTimer /></div>}
                            {swapFor === ex.k && !subKey && !entry.kit && (
                              <div style={{ background: C.panel3, borderRadius: 12, padding: "11px 12px", display: "grid", gap: 8 }}>
                                {memKey && (EX_LIB[memKey] || data.customEx?.[memKey]) && (
                                  <button onClick={() => chooseSub(memKey)} style={{ textAlign: "left", background: C.sageSoft,
                                    border: `1px solid ${C.sage}`, borderRadius: 10, padding: "9px 12px", cursor: "pointer" }}>
                                    <span style={{ fontFamily: MONO, fontSize: 9, color: C.sage, letterSpacing: "0.1em" }}>LAST TIME · </span>
                                    <span style={{ color: C.ink, fontSize: 13 }}>{(EX_LIB[memKey] || data.customEx?.[memKey]).n}</span></button>)}
                                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                                  <button onClick={() => setEqSel(null)} style={{ padding: "6px 11px", borderRadius: 12, border: "none",
                                    cursor: "pointer", background: !eqSel ? C.amber : C.panel2, color: !eqSel ? C.ground : C.dim,
                                    fontFamily: MONO, fontSize: 10 }}>ALL</button>
                                  {eqs.map((e) => (
                                    <button key={e} onClick={() => setEqSel(eqSel === e ? null : e)} style={{ padding: "6px 11px",
                                      borderRadius: 12, border: "none", cursor: "pointer",
                                      background: eqSel === e ? C.amber : C.panel2, color: eqSel === e ? C.ground : C.dim,
                                      fontFamily: MONO, fontSize: 10 }}>{e}</button>))}
                                </div>
                                <div style={{ display: "grid", gap: 6 }}>
                                  {options.filter((o) => !eqSel || (o.eq || "Other") === eqSel).map((o) => (
                                    <button key={o.k} onClick={() => chooseSub(o.k)} style={{ textAlign: "left",
                                      background: C.panel2, border: "none", borderRadius: 10, padding: "9px 12px", cursor: "pointer" }}>
                                      <span style={{ color: C.ink, fontSize: 13 }}>{o.n}</span>
                                      <span style={{ fontFamily: MONO, fontSize: 9, color: C.dimmer, marginLeft: 8 }}>{(o.eq || "OTHER").toUpperCase()}</span>
                                    </button>))}
                                </div>
                                <div style={{ display: "flex", gap: 6 }}>
                                  <input value={customTxt} onChange={(e) => setCustomTxt(e.target.value)}
                                    placeholder="Or type what you did instead…"
                                    style={{ flex: 1, background: C.panel2, border: "none", borderRadius: 10, color: C.ink,
                                      fontFamily: SANS, fontSize: 13, padding: "9px 11px", outline: "none" }} />
                                  <button disabled={!customTxt.trim()} onClick={() => {
                                      const nm = customTxt.trim();
                                      const key = "cu_" + nm.toLowerCase().replace(/[^a-z0-9]+/g, "_").slice(0, 24);
                                      const day = data.days[cursor] || {};
                                      persist({ ...data,
                                        customEx: { ...(data.customEx || {}), [key]: { n: nm, eq: "Custom", inc: 2.5, m: ex.m } },
                                        days: { ...data.days, [cursor]: { ...day, subs: { ...(day.subs || {}), [ex.k]: key } } },
                                        subMemory: { ...(data.subMemory || {}), [ex.k]: key } });
                                      setSwapFor(null); setEqSel(null); setCustomTxt("");
                                    }} style={{ padding: "0 14px", borderRadius: 10, border: "none",
                                      cursor: customTxt.trim() ? "pointer" : "default",
                                      background: customTxt.trim() ? C.terra : C.panel2,
                                      color: customTxt.trim() ? C.ground : C.dimmer,
                                      fontFamily: MONO, fontSize: 11, fontWeight: 700 }}>USE</button>
                                </div>
                              </div>)}
        ...
        {tab === "lifts" && (() => {
          ...
          const seen = new Set();
          groups.forEach(([g]) =>
            [[SESSIONS_1[g], "B1"], [SESSIONS_2[g], "B2"], [PPL_A[g], "B3"], [PPL_B[g], "B4"], [PPL_C[g], "B5"]].forEach(([s, ph]) =>
              s.ex.forEach((ex) => {
                if (seen.has(ex.k)) { const r = allEx.find((x) => x.k === ex.k); if (r) r.ph = r.ph === ph ? ph : "ROTATES"; return; }
                seen.add(ex.k); allEx.push({ ...ex, ph, g });
              })));
          Object.keys(SUB_LIB).forEach((k) => {
            if (!seen.has(k) && liftRecords[k]) { seen.add(k); allEx.push({ k, ...SUB_LIB[k], ph: "SUB", g: "subs" }); } });
          Object.keys(data.customEx || {}).forEach((k) => {
            if (!seen.has(k) && liftRecords[k]) { seen.add(k); allEx.push({ k, ...data.customEx[k], ph: "CUSTOM", g: "subs" }); } });
          const subsLogged = allEx.filter((ex) => ex.g === "subs");
            ...
            <div style={{ marginTop: 22, display: "grid", gap: 14 }}>
              ...
              <Panel glow>
                ...
                {(() => {
                  ...
                  const fresh = Object.values(liftRecords).filter((r) => r.pr && r.pr.date >= cutoff).length;
                  return fresh > 0 ? (
                    <div style={{ fontFamily: MONO, fontSize: 11, color: "var(--pop)", marginBottom: 8 }}>
                      ✦ {fresh} PR{fresh === 1 ? "" : "S"} SET THIS FORTNIGHT</div>) : null;
                })()}
              ...
              {filtered ? (
                ...
                [...groups, ...(subsLogged.length ? [["subs", "Substitutes & custom", C.amber]] : [])].map(([g, label, col]) => {
        ...
        {showSettings && (
          <div onClick={() => setShowSettings(false)} style={{ position: "fixed", inset: 0, zIndex: 100,
            background: "rgba(0,0,0,0.55)", display: "grid", placeItems: "center", padding: 16 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, maxHeight: "88vh", overflowY: "auto",
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                <Label style={{ marginBottom: 0 }}>Appearance</Label>
                <button onClick={() => setShowSettings(false)} style={{ ...navBtn, padding: "5px 12px" }}>✕</button>
              </div>
              <div style={{ fontSize: 12, color: C.dim, marginBottom: 12 }}>The five themes from the Design round — every screen re-themes instantly.</div>
              <div style={{ display: "grid", gap: 8 }}>
                {Object.entries(THEMES).map(([id, t]) => {
                  const on = (data.theme || null) === id || (!data.theme && id === themeId);
                  return (
                    <div key={id} onClick={() => setField("theme", id)} style={{ display: "flex", gap: 12, alignItems: "center",
                      padding: "11px 13px", borderRadius: 12, cursor: "pointer", background: C.panel2,
                      border: on ? "2px solid var(--acc)" : `1px solid ${C.ruleSoft}` }}>
                      <div style={{ width: 58, height: 38, borderRadius: 8, background: t.swBg, flexShrink: 0,
                        display: "flex", alignItems: "center", justifyContent: "center", gap: 3, border: `1px solid ${C.rule}` }}>
                        {t.sw.map((c, i) => <span key={i} style={{ width: 8, height: [17, 11, 14][i], borderRadius: 2, background: c }} />)}
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, fontWeight: 600 }}>{t.label}
                          {t.tag && <span style={{ fontFamily: MONO, fontSize: 8, letterSpacing: "0.1em", marginLeft: 6,
                            background: "var(--acc)", color: "var(--accInk)", borderRadius: 4, padding: "2px 5px" }}>{t.tag}</span>}</div>
                        <div style={{ fontSize: 10.5, color: C.dim }}>{t.desc}</div>
                      </div>
                      {on && <span style={{ color: "var(--acc)", fontFamily: MONO, fontSize: 13 }}>✓</span>}
                    </div>);
                })}
              </div>
              <div onClick={() => persist({ ...data, theme: null, autoDark: data.autoDark === false ? true : false })}
                style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 14px",
                  borderRadius: 12, background: C.panel2, marginTop: 10, cursor: "pointer" }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>Auto dark mode</div>
                  <div style={{ fontSize: 10.5, color: C.dim }}>Field Ledger by day → Midnight Signal after 19:00 (clears manual pick)</div>
                </div>
                <div style={{ width: 40, height: 23, borderRadius: 12, position: "relative", flexShrink: 0,
                  background: (!data.theme && data.autoDark !== false) ? "var(--acc)" : C.rule, transition: "background .25s" }}>
                  <span style={{ position: "absolute", top: 2.5, left: (!data.theme && data.autoDark !== false) ? 19 : 3,
                    width: 18, height: 18, borderRadius: "50%", background: C.panel, transition: "left .25s" }} /></div>
              </div>
            </div>
          </div>
        )}
...
/* Ellipse body maps — geometry from the Design prototype */
const BODY_FRONT = [
  { m: "shoulders", cx: 32, cy: 44, rx: 7, ry: 5 }, { m: "shoulders", cx: 68, cy: 44, rx: 7, ry: 5 },
  { m: "chest", cx: 50, cy: 54, rx: 13, ry: 8 },
  { m: "biceps", cx: 27, cy: 63, rx: 4.5, ry: 9 }, { m: "biceps", cx: 73, cy: 63, rx: 4.5, ry: 9 },
  { m: "core", cx: 50, cy: 78, rx: 10, ry: 12 },
  { m: "grip", cx: 23, cy: 84, rx: 4, ry: 9 }, { m: "grip", cx: 77, cy: 84, rx: 4, ry: 9 },
  { m: "quads", cx: 41, cy: 120, rx: 7, ry: 17 }, { m: "quads", cx: 59, cy: 120, rx: 7, ry: 17 },
  { m: "calves", cx: 41, cy: 158, rx: 5, ry: 11 }, { m: "calves", cx: 59, cy: 158, rx: 5, ry: 11 },
];
const BODY_BACK = [
  { m: "upperback", cx: 50, cy: 50, rx: 13, ry: 9 },
  { m: "triceps", cx: 27, cy: 62, rx: 4.5, ry: 9 }, { m: "triceps", cx: 73, cy: 62, rx: 4.5, ry: 9 },
  { m: "lats", cx: 37, cy: 66, rx: 6, ry: 10 }, { m: "lats", cx: 63, cy: 66, rx: 6, ry: 10 },
  { m: "glutes", cx: 50, cy: 95, rx: 11, ry: 8 },
  { m: "hamstrings", cx: 41, cy: 122, rx: 6.5, ry: 15 }, { m: "hamstrings", cx: 59, cy: 122, rx: 6.5, ry: 15 },
  { m: "calves", cx: 41, cy: 157, rx: 5, ry: 11 }, { m: "calves", cx: 59, cy: 157, rx: 5, ry: 11 },
];

function BodyFig({ regions, sel, onSel, label }) {
  return (
    <div style={{ textAlign: "center" }}>
      <svg viewBox="0 0 100 196" width="112" height="219">
        <circle cx="50" cy="20" r="10" fill="none" stroke={C.rule} strokeWidth="1.5" />
        <path d="M35,34 L65,34 L70,90 L60,96 L60,140 L64,178 M36,178 L40,140 L40,96 L30,90 Z"
          fill="none" stroke={C.rule} strokeWidth="1.5" />
        {regions.map((r, i) => {
          const active = sel === r.m;
          return <ellipse key={i} cx={r.cx} cy={r.cy} rx={r.rx} ry={r.ry}
            onClick={() => onSel(active ? null : r.m)}
            fill={active ? "var(--acc)" : "var(--card2)"}
            stroke={active ? "var(--acc)" : "var(--line)"} strokeWidth="1"
            style={{ cursor: "pointer", opacity: 0.88 }} />;
        })}
        <text x="50" y="193" textAnchor="middle" fontSize="8" fill="var(--mut)" fontFamily="ui-monospace,monospace">{label}</text>
      </svg>
    </div>
  );
}
